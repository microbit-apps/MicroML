...
static float *malloc_and_fill_random(int len) {
    ...
    const int range = 2;
    for (int i = 0; i < len; i++)
        arr[i] = (float)random(range + 1) - 1.0f; // +1, since random(range) returns [0, range-1]
    return arr;
}
...
// void construct_nn(int *layer_dims, int layer_dims_len, int *activation_fn_enums, TestTrainSplitDataset* ds) {
...
void construct_nn(Buffer layer_dims, Buffer activation_fn_enums, DatasetEnum dataset_enum) {
    ...
    for (int i = 0; i < num_layers; i++) {
      ...
      layers[i] = construct_layer(in_dim, out_dim,
                                  (ActivationFunctionEnum)activation_fn_enums->data[i]);
    }
...
static float *unpack_buffer_into_floats(Buffer buf) {
    ...
    for (int i = 0; i < count; i++)
        out[i] = toFloat(getNumberCore(buf->data + i * s, s, NumberFormat::Float32LE));
    return out;
}
